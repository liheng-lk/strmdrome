...
func (e *ffmpeg) Probe(ctx context.Context, files []string) (string, error) {
	...
	// Filter out .strm files
	var filtered []string
	for _, f := range files {
		if !strings.HasSuffix(strings.ToLower(f), ".strm") {
	...
	args := createProbeCommand(probeCmd, filtered)
...
func (e *ffmpeg) ProbeAudioStream(ctx context.Context, filePath string) (*AudioProbeResult, error) {
	...
	// .strm 文件拦截：伪造 ffprobe 结果，绕过由于探测非媒体文件而导致的“转码决策失败”错误。
	if strings.HasSuffix(strings.ToLower(filePath), ".strm") {
		log.Debug("ffmpeg: Mocking ffprobe result for .strm file", "path", filePath)
