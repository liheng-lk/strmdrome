...
func (pub *Router) handleStream(w http.ResponseWriter, r *http.Request) {
	...
	// [STRM-V4] Try redirection before starting the stream
	ok, err := pub.alist.TryRedirect(w, r, mf)
	if err != nil {
		log.Error(ctx, "STRM-V4: Redirection failed", "id", mf.ID, err)
		http.Error(w, "AList tracking failed", http.StatusInternalServerError)
		return
	}
	if ok {
		return
