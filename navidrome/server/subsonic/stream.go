...
func (api *Router) Stream(w http.ResponseWriter, r *http.Request) (*responses.Subsonic, error) {
	...
	log.Warn("Subsonic: [STRM-HEARTBEAT-1.5.2] Stream check", "id", id, "suffix", mf.Suffix)
...
func (api *Router) Download(w http.ResponseWriter, r *http.Request) (*responses.Subsonic, error) {
	...
	case *model.MediaFile:
		if strings.HasSuffix(strings.ToLower(v.Path), ".strm") {
			...
			if err != nil {
				log.Error("Download: Failed to read .strm file", "path", v.AbsolutePath(), "err", err)
			} else {
				urlStr := strings.TrimSpace(string(content))
				if strings.HasPrefix(urlStr, "http") {
					log.Info("Download: Redirecting .strm to remote URL", "id", id, "url", urlStr)
					http.Redirect(w, r, urlStr, http.StatusFound)
					return nil, nil
				} else {
					log.Warn("Download: .strm file does not contain a valid URL", "path", v.AbsolutePath(), "content", urlStr)
				}
			}
		}
