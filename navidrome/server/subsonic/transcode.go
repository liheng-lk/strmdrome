...
func (api *Router) GetTranscodeDecision(w http.ResponseWriter, r *http.Request) (*responses.Subsonic, error) {
	...
	log.Warn(ctx, "[STRM-V2] Decision check for suffix", "id", mediaID, "suffix", mf.Suffix)

	var decision *stream.TranscodeDecision
	// .strm 文件拦截：强制允许直接播放，避开系统的转码决策（防止 ffprobe 探测失败）
	if strings.EqualFold(mf.Suffix, "strm") {
		log.Warn(ctx, "[STRM-V2] Forcing Direct Play for .strm file", "mediaID", mediaID)
		
		finalDuration := float32(mf.Duration)
		// 如果数据库没有时长，尝试从 AList 实时解析
		if finalDuration <= 0 {
			filePath := mf.AbsolutePath()
			if contentBytes, err := os.ReadFile(filePath); err == nil {
				targetPath := strings.TrimSpace(string(contentBytes))
				if !strings.Contains(targetPath, "://") && (strings.HasPrefix(targetPath, "/") || !strings.Contains(targetPath, ".")) {
					if info, err := api.alist.GetDirectLink(targetPath, ""); err == nil && info.Duration > 0 {
						log.Warn(ctx, "[STRM-V2] Resolved missing duration from AList", "id", mediaID, "duration", info.Duration)
						finalDuration = float32(info.Duration)
					}
				}
			}
		}
		...
		// Make the decision for regular files
		var err error
...
func (api *Router) GetTranscodeStream(w http.ResponseWriter, r *http.Request) (*responses.Subsonic, error) {
	...
	// [STRM-V2] Try redirection before starting the stream
	ok, err := api.alist.TryRedirect(w, r, mf)
	if err != nil {
		log.Error(ctx, "STRM-V2: Redirection failed", "id", mf.ID, err)
	}
	if ok {
		return nil, nil
	}
