...
func (e extractor) extractMetadata(filePath string) (info *metadata.Info, err error) {
	...
	if err != nil {
		// .strm 文件: 远端 ID3 获取失败，不 Skip 文件，而是返回空元数据
		// Navidrome 会自动回退到用文件名/路径解析 Artist、Album、Title
		if strings.HasSuffix(strings.ToLower(filePath), ".strm") {
			log.Debug("gotaglib: .strm remote tag fetch failed, deducting metadata from path", "filePath", filePath)
			
			// Deduct metadata from path: Artist/Album/Title.strm
